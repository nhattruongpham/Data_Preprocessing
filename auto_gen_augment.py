    ...
    def __call__(self, img, bboxes):
            ...
            if bboxes.any() == True:
                ...
                if random.random() < self.p:
                    ...
                    bboxes[:, [1, 3]] = bboxes[:, [1, 3]] + 2*(img_center[[1, 3]] - bboxes[:, [1, 3]])

                    box_h = abs(bboxes[:, 1] - bboxes[:, 3])

                    bboxes[:, 1] = bboxes[:, 1] - box_h
                    bboxes[:, 3] = bboxes[:, 3] + box_h
            else:
                bboxes = None
                img = None
...
class RandomNoise(object):
    def __init__(self, typical = (1, 6)):
        self.typical = typical
    
    def __call__(self, img, bboxes):
        num = random.randint(*self.typical)
        if num == 1 or num == 4:
            noise_type = "gauss"
        elif num == 2 or num == 5:
            noise_type = "poisson"
